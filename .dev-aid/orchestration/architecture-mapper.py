...
class ArchitectureMapper:
    ...
    def scan_directory(self, directory: Path, extensions: List[str] = None):
        ...
        for ext in extensions:
            for file_path in directory.rglob(f'*{ext}'):
                ...
                if any(part in file_path.parts for part in ['node_modules', 'venv', '.venv', '__pycache__', 'dist', 'build']):
                    continue
    ...
    def run(self, target: Optional[Path] = None, diagram_type: str = "all", output_file: Optional[Path] = None):
        """Main execution"""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate architecture diagrams from codebase"
    )
    parser.add_argument(
        "target",
        nargs="?",

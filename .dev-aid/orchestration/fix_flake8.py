...
def fix_f841_unused_variables(filepath):
    ...
    content = re.sub(r'([a-zA-Z_]+) = ', lambda m: f'_{m.group(1)} = ' if 'client' in m.group(1).lower() or m.group(1) == 'e' else m.group(0), content)
...
unused_imports = {
    ...
    'tests/test_cost_tracker.py': ['json', 'datetime', 'Path', 'MagicMock', 'patch', 'RoutingDecision'],

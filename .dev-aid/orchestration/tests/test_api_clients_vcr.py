...
HAS_API_KEYS = bool(os.getenv("ANTHROPIC_API_KEY") or os.getenv("OPENAI_API_KEY") or os.getenv("GOOGLE_API_KEY"))

skip_if_no_setup = pytest.mark.skipif(
    not (HAS_CASSETTES or HAS_API_KEYS),
    reason="No cassettes or API keys available"
)
...
class TestAnthropicClientVCR:
    ...
    def test_long_context(self, mock_model_config):
        ...
        messages = [
            ...
            Message(role="assistant", content="Quantum computing uses quantum mechanics principles..."),
...
class TestCrossProviderConsistency:
    ...
    def test_response_structure(self, mock_model_config, mock_google_config, mock_openai_config):
        ...
        if os.getenv("ANTHROPIC_API_KEY") or (CASSETTES_DIR / "anthropic_simple_request.yaml").exists():
            providers.append(("anthropic", AnthropicClient(os.getenv("ANTHROPIC_API_KEY", "test"), mock_model_config)))

        if os.getenv("GOOGLE_API_KEY") or (CASSETTES_DIR / "google_simple_request.yaml").exists():
            providers.append(("google", GoogleClient(os.getenv("GOOGLE_API_KEY", "test"), mock_google_config)))

        if os.getenv("OPENAI_API_KEY") or (CASSETTES_DIR / "openai_simple_request.yaml").exists():
            providers.append(("openai", OpenAIClient(os.getenv("OPENAI_API_KEY", "test"), mock_openai_config)))

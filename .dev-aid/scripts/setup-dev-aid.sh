...
if should_run_phase 8; then
    ...
    echo -e "${GREEN}✅ Dev-AID is ready.${NC}"
    echo ""
    echo -e "${CYAN}Next steps (in order):${NC}"
    echo ""
    echo "  1. Add API keys (if you skipped them in the wizard):"
    echo -e "     ${YELLOW}edit .dev-aid/config/.env${NC}"
    echo ""
    echo "  2. Open your AI coding tool in this directory:"
    echo -e "     ${YELLOW}claude${NC}     (Claude Code)"
    echo -e "     ${YELLOW}gemini${NC}    (Gemini CLI)"
    echo -e "     ${YELLOW}codex${NC}     (Codex CLI)"
    echo ""
    echo "  3. Try your first command:"
    echo -e "     ${YELLOW}/aid-help${NC}     — list every available /aid-* command"
    echo -e "     ${YELLOW}/aid-status${NC}   — show Dev-AID configuration and health"
    echo -e "     ${YELLOW}/aid-skills${NC}   — see which skills are installed"
    echo -e "     ${YELLOW}/aid-pr 1${NC}     — review PR #1 (if your repo has one)"
    echo ""
    echo -e "${CYAN}Need help?${NC}"
    echo "  Beta testing guide: .dev-aid/docs/BETA-TESTING-GUIDE.md"
    echo "  File a bug:         https://github.com/Probably-Group/Dev-AID/issues/new?template=beta_feedback.yml"
    echo "  Reconfigure:        ./.dev-aid/scripts/setup-dev-aid.sh"
    echo "  Uninstall cleanly:  ./.dev-aid/scripts/uninstall-dev-aid.sh"
    echo ""

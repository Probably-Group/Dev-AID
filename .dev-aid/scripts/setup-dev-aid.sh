...
if should_run_phase 8; then
    ...
    echo "     ${YELLOW}edit .dev-aid/config/.env${NC}"
    echo ""
    echo "  2. Open your AI coding tool in this directory:"
    echo "     ${YELLOW}claude${NC}     (Claude Code)"
    echo "     ${YELLOW}gemini${NC}    (Gemini CLI)"
    echo "     ${YELLOW}codex${NC}     (Codex CLI)"
    echo ""
    echo "  3. Try your first command:"
    echo "     ${YELLOW}/aid-help${NC}     — list every available /aid-* command"
    echo "     ${YELLOW}/aid-status${NC}   — show Dev-AID configuration and health"
    echo "     ${YELLOW}/aid-skills${NC}   — see which skills are installed"
    echo "     ${YELLOW}/aid-pr 1${NC}     — review PR #1 (if your repo has one)"
    echo ""
